...
| `stroke.ts` (`strokeToCss`) | [x] | `style/stroke.ts` | 实线、`strokeAlign` (INSIDE→border / OUTSIDE→box-shadow / CENTER→border)、`individualStrokeWeights`;**`strokeDashes` 自定义 `[dash, gap, ...]`** 通过 `border-image` + `repeating-linear-gradient` 还原(水平边精准,垂直边因 CSS border-image 切片机制为已知近似);OUTSIDE 因 `box-shadow` 不支持虚线,保持实线兜底 |
